  ...
  try {
    ...
    const posts = files.map((file) => {
      try {
        ...
        /* main_page_feature pins a post into the homepage "What's new" band.
           Separate from `featured`, which only controls the blog page's own
           hero slot — a post can be one, the other, or both. */
        const isHomeFeatured =
          data.main_page_feature === true ||
          String(data.main_page_feature).toLowerCase() === 'true';
        ...
        return {
          ...
          featured: isFeatured,
          mainPageFeature: isHomeFeatured,
          draft: isDraft,
          date: dateVal,
...
/* The homepage "What's new" band: posts explicitly opted in with
   main_page_feature: true in their frontmatter, newest first. Drafts never
   qualify. Nothing opted in means the homepage hides the section rather
   than falling back to whatever happens to be newest. */
export function getHomeFeaturedPosts(limit = 3) {
  const picked = getPublishedPosts().filter((p) => p.mainPageFeature);
