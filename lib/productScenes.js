...
function makeLineMaterial(opacity) {
  return new LineBasicMaterial({ color: EDGE_COLOR, transparent: true, opacity });
}
function makePulseMesh() {
  return new Mesh(new SphereGeometry(0.045, 8, 8), new MeshBasicMaterial({ color: EDGE_COLOR }));
}
...
function buildNavnet() {
  ...
  const hub = new Mesh(new IcosahedronGeometry(0.22, 0), material);
  group.add(hub);

  const deviceGeometry = new IcosahedronGeometry(0.09, 0);
  const deviceCount = 7;
  const devices = Array.from({ length: deviceCount }, (_, i) => {
    const angle = (i / deviceCount) * Math.PI * 2;
    const y = ((i % 3) - 1) * 0.32;
    return new Vector3(Math.cos(angle) * 1.0, y, Math.sin(angle) * 1.0);
  });
  devices.forEach((pos) => {
    const device = new Mesh(deviceGeometry, material);
  ...
  group.add(new LineSegments(lineGeometry, makeLineMaterial(0.55)));
...
function buildPlatform() {
  const group = new Group();
  const material = makeMaterial();
  const nodeGeometry = new IcosahedronGeometry(0.1, 0);
  const apexGeometry = new IcosahedronGeometry(0.14, 0);
  ...
  const apexPos = new Vector3(0, 0.7, 0);
  const apex = new Mesh(apexGeometry, material);
  ...
  const lineGeometry = new BufferGeometry();
  lineGeometry.setAttribute('position', new Float32BufferAttribute(linePositions, 3));
  group.add(new LineSegments(lineGeometry, makeLineMaterial(0.75)));
...
function buildReserved() {
  ...
  const root = new LineSegments(new EdgesGeometry(geometry), makeLineMaterial(0.6));
  return { root };
}
...
function createInstance(canvas) {
  ...
  scene.add(new AmbientLight(0x101420, 0.9));
  const key = new DirectionalLight(KEY_COLOR, 1.15);
  key.position.set(2.2, 3, 2.5);
  scene.add(key);
  const fill = new DirectionalLight(FILL_COLOR, 0.3);
